...
const Keyboard = (props) => {
  ...
  const mathOperBtnHandler = (name) => {
    if (oper.length > 14)
      return Alert.alert("Warning", "characters must be less than 15");
    if (isNaN(oper[oper.length - 1])) return;
    if (name == "+") {
      setOper(oper + "+");
    }
    if (name == "-") {
  ...
  const extraNumOper = (name) => {
    if (name == ".") {
      if (oper.length > 0 && !isNaN(oper[oper.length - 1])) setOper(oper + ".");
      if (oper.length == 0) setOper(oper + "0.");
      return;
    }
    if (name == "+/-") {
      if (oper.slice(0, 5) !== "(-1)*") {
  ...
  return (
    <View style={[styles.keyboard, styles.column]}>
      ...
      <View style={[styles.keyGroup, styles.row, styles.between]}>
        ...
        <TouchableOpacity
          style={styles.btn}
          onPress={() => extraMathOper("square")}
        >
          <MaterialCommunityIcons name="square-root" size={40} color="#000" />
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.btn}
          onPress={() => extraMathOper("residual")}
        >
